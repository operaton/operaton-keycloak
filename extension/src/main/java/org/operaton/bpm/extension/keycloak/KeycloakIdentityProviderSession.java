...
public class KeycloakIdentityProviderSession implements ReadOnlyIdentityProvider {
  ...
  public UserQuery createUserQuery() {
    return new KeycloakUserQuery(
        org.operaton.bpm.engine.impl.context.Context.getProcessEngineConfiguration().getCommandExecutorTxRequired());
  }
  ...
  private boolean doCheckPassword(String userId, String password) {
    ...
    try {
      ...
      HttpEntity<String> request = new HttpEntity<>(
          "client_id=" + keycloakConfiguration.getClientId() + "&client_secret="
              + keycloakConfiguration.getClientSecret() + "&username=" + userName + "&password=" + URLEncoder.encode(
              password, keycloakConfiguration.getCharset()) + "&grant_type=password", headers);
      restTemplate.postForEntity(keycloakConfiguration.getKeycloakIssuerUrl() + "/protocol/openid-connect/token",
      ...
      throw new IdentityProviderException(
          "Unable to authenticate user at " + keycloakConfiguration.getKeycloakIssuerUrl(), hcee);
    ...
    try {
      if (keycloakConfiguration.isUseEmailAsOperatonUserId()) {
        ResponseEntity<String> response = restTemplate.exchange(
            keycloakConfiguration.getKeycloakAdminUrl() + "/users?exact=true&email=" + userId, HttpMethod.GET,
            String.class);
      ...
      throw new KeycloakUserNotFoundException(userId + (keycloakConfiguration.isUseEmailAsOperatonUserId() ?
