...
import static org.operaton.bpm.extension.keycloak.json.JsonUtil.findFirst;
...
public class KeycloakUserService extends KeycloakServiceBase {
   ...
   */
  public KeycloakUserService(KeycloakConfiguration keycloakConfiguration,
  ...
  public String getKeycloakAdminUserId(String configuredAdminUserId) {
    try {
      ...
      try {
        ResponseEntity<String> response = restTemplate.exchange(
            keycloakConfiguration.getKeycloakAdminUrl() + "/users?exact=true&username=" + configuredAdminUserId,
      ...
      throw new IdentityProviderException(
          "Configured administratorUserId " + configuredAdminUserId + " does not exist.");
    } catch (RestClientException rce) {
      throw new IdentityProviderException(
          "Unable to read data of configured administratorUserId " + configuredAdminUserId, rce);
    }
  }
  ...
  public List<User> requestUsersByGroupId(CacheableKeycloakUserQuery query) {
    ...
    try {
      ...
      ResponseEntity<String> response = restTemplate.exchange(
          keycloakConfiguration.getKeycloakAdminUrl() + "/groups/" + keyCloakID + "/members?max="
              + getMaxQueryResultSize(), HttpMethod.GET, String.class);
      if (!response.getStatusCode().equals(HttpStatus.OK)) {
        throw new IdentityProviderException(
            "Unable to read group members from " + keycloakConfiguration.getKeycloakAdminUrl() + ": HTTP status code "
      ...
      for (int i = 0; i < searchResult.size(); i++) {
        ...
        if (keycloakConfiguration.isUseEmailAsOperatonUserId() && !StringUtils.hasLength(
            getJsonString(keycloakUser, MEMBER_EMAIL))) {
          continue;
        }
        if (keycloakConfiguration.isUseUsernameAsOperatonUserId() && !StringUtils.hasLength(
            getJsonString(keycloakUser, MEMBER_USERNAME))) {
          continue;
        }
  ...
  public List<User> requestUsersWithoutGroupId(CacheableKeycloakUserQuery query) {
    ...
    try {
      ...
      if (!response.getStatusCode().equals(HttpStatus.OK)) {
        throw new IdentityProviderException(
            "Unable to read users from " + keycloakConfiguration.getKeycloakAdminUrl() + ": HTTP status code "
                + response.getStatusCode().value());
      }
  ...
  private ResponseEntity<String> requestUserById(String userId) throws RestClientException {
    try {
      ...
      ResponseEntity<String> response = restTemplate.exchange(keycloakConfiguration.getKeycloakAdminUrl() + userSearch,
          HttpMethod.GET, String.class);
      String result =
          keycloakConfiguration.isUseEmailAsOperatonUserId() || keycloakConfiguration.isUseUsernameAsOperatonUserId() ?
              response.getBody() :
              "[" + response.getBody() + "]";
