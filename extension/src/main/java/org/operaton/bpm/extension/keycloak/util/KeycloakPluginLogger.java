package org.operaton.bpm.extension.keycloak.util;
...
public class KeycloakPluginLogger extends BaseLogger {
  ...
  public static final KeycloakPluginLogger INSTANCE = BaseLogger.createLogger(KeycloakPluginLogger.class, PROJECT_CODE,
      "org.operaton.bpm.extension.keycloak", "01");
  ...
  public void grantGroupPermissions(String administratorGroupName, String administratorGroupId, String resourceName) {
    logInfo("002", "GRANT group {}[{}] ALL permissions on resource {}.", administratorGroupName, administratorGroupId,
        resourceName);
  }
  ...
  public void activationError(String pluginClassName, String engineName, String errorMessage) {
    logError("005", "PLUGIN {} could not be activated on process engine {}: {}", pluginClassName, engineName,
        errorMessage);
  }
  ...
  public void requestFailedUnauthorized(String request) {
    logWarn("013",
        "REQUEST {} failed unauthorized - auto retry once with new token. Please check access token lifespan, session timeouts, etc.",
        request);
  }

  public void missingRefreshToken() {
    logWarn("014",
        "TOKEN refresh not possible - please activate the 'Use Refresh Tokens For Client Credentials Grant' option.");
  }
