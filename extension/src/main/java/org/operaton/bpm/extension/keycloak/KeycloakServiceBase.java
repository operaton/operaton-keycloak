...
public abstract class KeycloakServiceBase {
  ...
  protected String getKeycloakUserID(String userId) throws KeycloakUserNotFoundException, RestClientException {
    ...
    try {
      URI uri = UriComponentsBuilder.fromUriString(
              keycloakConfiguration.getKeycloakAdminUrl() + userSearch + URLEncoder.encode(userId, StandardCharsets.UTF_8))
          .build(true)
          .toUri();
      ...
      JsonObject result = findFirst(resultList,
          keycloakConfiguration.isUseUsernameAsOperatonUserId() ? "username" : "email", userId);
      if (result != null) {
   ...
   */
  protected boolean matches(Object[] queryParameter, Object attribute) {
    return queryParameter == null || queryParameter.length == 0 || (attribute != null && Arrays.asList(queryParameter)
        .contains(attribute));
  }
  ...
  protected boolean isAuthorized(Permission permission, Resource resource, String resourceId) {
    return !keycloakConfiguration.isAuthorizationCheckEnabled()
        || org.operaton.bpm.engine.impl.context.Context.getCommandContext()
